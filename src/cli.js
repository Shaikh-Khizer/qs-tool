...
Shorthand flags (no sub-command needed):
  qs-tool -p 'a[b]=1'                       parse  (query string → JSON)
  qs-tool -s '{"a":{"b":1}}'                stringify (JSON → query string)
  qs-tool -p -c 'a=1&b=2'                   compact JSON output
  qs-tool -s -a repeat '{"t":["x","y"]}'    array as repeated keys (t=x&t=y)
  qs-tool -p -f req.txt                     parse from file
  cat data.txt  | qs-tool -p                parse from stdin
  cat body.json | qs-tool -s                stringify from stdin
  cat body.json | qs-tool -s -a indices     stringify with indexed arrays

Sub-commands (explicit):
  qs-tool parse 'a[b][c]=1&a[b][d][]=2'
  qs-tool stringify '{"email":{"user":"khizer@gmail.com"}}'
